...
use crate::{Orchestration, Entrant, Schedule};
...
impl Tournament {
    pub fn do_action(&mut self, action: TournamentActions) -> TournamentViewModel {
        match action {
            TournamentActions::ChangeName(name) => self.name = name,
        }
        self.render_view_model()
    }
    pub fn render_view_model(&self) -> TournamentViewModel {
        todo!()
    }
}
