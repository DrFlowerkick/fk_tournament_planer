...
async fn given_filter_and_limit_when_list_tournament_bases_then_db_fake_results_are_forwarded() {
    ...
    // Act
    let got = core
        .list_tournament_bases(sport_id, Some("ma"), None, false, Some(2))
        .await
        .expect("db ok");

    // Assert: exactly 2 with names containing "ma" (case-insensitive)
    assert_eq!(got.len(), 2);
    let names: Vec<String> = got.iter().map(|t| t.get_name().to_string()).collect();
...
async fn given_only_limit_when_list_tournament_bases_then_limit_is_respected() {
    ...
    let got = core
        .list_tournament_bases(sport_id, None, None, false, Some(3))
        .await
        .expect("db ok");
    assert_eq!(got.len(), 3);
}
...
async fn given_db_fake_failure_when_list_tournament_bases_then_error_propagates() {
    ...
    let err = core
        .list_tournament_bases(Uuid::new_v4(), None, None, false, None)
        .await
        .expect_err("expected DB error");
