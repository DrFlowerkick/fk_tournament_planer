//! Concurrency tests for the SportConfig DB adapter.
...
async fn given_two_parallel_updates_from_v0_then_only_one_succeeds_and_version_is_1() -> Result<()>
{
    ...
    let candidate_b = mutate_sport_config_v3(v0.clone());
    ...
    let h1 = tokio::spawn(async move {
        b1.wait().await;
        db1.save_sport_config(&candidate_a2).await
    });
    ...
    let h2 = tokio::spawn(async move {
        b2.wait().await;
        db2.save_sport_config(&candidate_b2).await
    });
    ...
    let fetched = db.get_sport_config(id).await?.expect("row must exist");
...
#[tokio::test(flavor = "multi_thread")]
async fn given_two_parallel_inserts_same_name_sport_then_only_one_succeeds_unique_violation_for_loser()
-> Result<()> {
    // Arrange: fresh DB
    init_db_testing();
    let tdb = TestDb::new().await?;
    ...
    let sport_id = Uuid::new_v4();
    let new_a = make_new_sport_config("concurrency-insert", sport_id);
    ...
    let h1 = tokio::spawn(async move {
        b1.wait().await;
        db1.save_sport_config(&new_a).await
    });

    let db2 = db.clone();
    let h2 = tokio::spawn(async move {
        b2.wait().await;
        db2.save_sport_config(&new_b).await
    });
