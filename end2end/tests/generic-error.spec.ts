...
test.describe("Generic error handling saving address", () => {
  test("shows a generic error banner on 500 server error", async ({ page }) => {
    ...
    await expect(page.getByTestId(T.banner.acknowledgmentNavigateBanner)).toBeVisible();
    await expect(page.getByTestId(T.banner.acknowledgmentNavigateBanner)).toContainText(
      "An unexpected error occurred during saving:"
    );
    await expect(page.getByTestId(T.banner.btnAcknowledgmentNavigateAction)).toBeVisible();
    await expect(page.getByTestId(T.banner.btnAcknowledgmentNavigate)).toBeVisible();
    ...
    await expect(page.getByTestId(T.banner.acknowledgmentNavigateBanner)).toBeHidden();
